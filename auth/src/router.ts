...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/auth/send-verification" && request.method === "POST") {
      ...
      if (!email) return new Response("Missing email", { status: 400, headers });
    ...
    if (url.pathname === "/api/auth/verify" && request.method === "POST") {
      ...
      if (!result.ok) {
        return new Response("Token expired or invalid", { status: 400, headers });
      }

      return new Response("Email verified!", { headers });
    }
