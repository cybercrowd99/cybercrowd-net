...
export async function onRequestPost(context) {
  ...
  if (!ALLOWED_CLOSE_REASONS.has(closeReason)) {
    return json({
      ok: false,
  ...
  await env.IDENTITY.put(
    "field:id:" + closedField.id,
    JSON.stringify(closedField),
    {
  ...
  await env.IDENTITY.put(
    "field-close:" + closeRecord.id,
    JSON.stringify(closeRecord),
    {
      expirationTtl: FIELD_TTL_SECONDS
    }
  );
  ...
  if (current && current.id === closedField.id) {
    await env.IDENTITY.put(
      "field:" + identityId,
      JSON.stringify(closedField),
      {
  ...
  await appendIndex(env, "field-close:index:field:" + field.id, closeRecord.id);
  await appendIndex(env, "field:index:status:closed", field.id);
  ...
  if (field.event_id) {
    await appendSync(env, field.event_id, {
...
export async function onRequestGet(context) {
  ...
  if (fieldId) {
    ...
    ids = await readIndex(env, "field-close:index:field:" + fieldId);
  } else {
  ...
  for (const id of ids) {
    ...
    if (!close) continue;
    if (close.identity_id !== identityId) continue;
...
async function readClose(env, closeId) {
  ...
  const raw = await env.IDENTITY.get("field-close:" + id);
