...
export async function onRequestPost(context) {
  ...
  if (!toIdentityId) {
    return json({ ok: false, error: "TARGET_IDENTITY_REQUIRED" }, 400);
  }

  const kind = cleanText(body.kind) || "human_selection";

  if (!ALLOWED_KINDS.has(kind)) {
    return json({ ok: false, error: "PING_KIND_NOT_ALLOWED" }, 400);
  }
  ...
  if (
    ...
    return json({ ok: false, error: "SELF_PING_BLOCKED" }, 400);
  }
  ...
  await env.IDENTITY.put("ping:" + ping.id, JSON.stringify(ping), {
